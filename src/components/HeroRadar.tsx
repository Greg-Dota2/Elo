...
import type { HeroRadarStats } from '@/lib/opendota'

const AXES = [
  { key: 'form',        label: 'Form',        desc: 'Win rate in recent matches' },
...
export default function HeroRadar({ stats }: { stats: HeroRadarStats }) {
  const [hovered, setHovered] = useState<number | null>(null)

  const chartVals = AXES.map(a => stats[a.key as keyof HeroRadarStats] as number)
  ...
  const activeRaw = activeAxis ? stats[activeAxis.key as keyof HeroRadarStats] as number : null

  return (
    <div className="rounded-2xl border border-border/60 bg-card/40 p-5 mb-6">
      <div className="flex items-center justify-between mb-5">
        <p className="section-label">Hero Performance</p>
        <span className="text-xs text-muted-foreground">Median across all public matches · OpenDota</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-6">
        ...
        <div className="shrink-0 relative">
          <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="w-[260px] sm:w-[320px]">
            {/* Grid rings */}
        ...
        {/* Stat cards */}
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-2">
          {AXES.map((axis, i) => {
            const raw = stats[axis.key as keyof HeroRadarStats] as number
