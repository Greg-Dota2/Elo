...
export default function RefreshOpenDotaButton() {
  ...
  async function handleClick() {
    ...
    const res = await fetch('/api/admin/refresh-opendota', { method: 'POST' })
    const data = await res.json()

    if (res.ok && data.ok) {
      setStatus('done')
      setMsg(`Done${data.errors ? ` (${data.errors} errors)` : ''}`)
      setLog(data.log ?? [])
    } else {
      setStatus('error')
    ...
    <div>
      ...
      >
        ...
        <div className="font-semibold text-sm">
          {status === 'loading' ? 'Refreshing... (~3 min)' : 'Refresh OpenDota'}
        </div>
        {msg && (
