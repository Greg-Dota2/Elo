...
export default function SeedEloButton() {
  const [status, setStatus] = useState<'idle' | 'loading' | 'done' | 'error'>('idle')
  const [msg, setMsg] = useState('')
  const [log, setLog] = useState<string[]>([])

  async function handleClick() {
    if (!confirm('This will import all 13 historical Tier 1 tournaments from OpenDota and recalculate ELO. It takes ~2 minutes. Continue?')) return
    setStatus('loading')
    setMsg('')
    setLog([])

    const res = await fetch('/api/elo/seed', { method: 'POST' })
    const data = await res.json()

    if (res.ok) {
      setStatus('done')
      setMsg(`${data.tournaments} tournaments · ${data.series_created} series · ${data.elo_matches_processed} ELO matches`)
      setLog(data.log ?? [])
    } else {
      setStatus('error')
    ...
    <div>
      ...
      >
        ...
        <div className="font-semibold text-sm">
          {status === 'loading' ? 'Importing (~2 min)...' : 'Seed Historical ELO'}
        </div>
        {msg && (
