...
export default function RecalcCorrectButton() {
  ...
  async function handleClick() {
    setStatus('loading')
    setMsg('')
    const res = await fetch('/api/admin/results/recalculate-correct', { method: 'POST' })
    const data = await res.json()
    if (res.ok) {
      setStatus('done')
      setMsg(`${data.fixed} matches fixed`)
    } else {
      setStatus('error')
  ...
  return (
    ...
    >
      ...
      <div className="font-semibold text-sm">
        {status === 'loading' ? 'Fixing...' : 'Fix Results'}
      </div>
      {msg && (
