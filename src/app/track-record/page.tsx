    ...
    <div className="fade-in-up max-w-3xl mx-auto py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 relative">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm leading-7 text-muted-foreground max-w-lg">
              Every pick is logged before the match starts — before the draft, before anyone has a read on the day.
              Nothing gets touched after. If I was wrong, it stays wrong, and you can see exactly where and when.
              That&apos;s the only way this means anything. If you can edit your picks after the result,
              you&apos;re not predicting — you&apos;re just rewriting history.
            </p>
          </div>
          ...
          <div className="shrink-0 flex flex-col items-center justify-center rounded-2xl px-7 py-5 text-center self-start sm:self-auto" style={{ background: accuracyBg, border: `1px solid ${accuracyBorder}` }}>
            ...
            <p className="text-[10px] text-muted-foreground mt-2">{totalCorrect} / {totalResolved} resolved</p>
          </div>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="rounded-2xl overflow-x-auto border border-border/50">
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <Link href={t?.slug ? `/tournaments/${t.slug}` : '/tournaments'} className="font-bold text-sm hover:text-primary transition-colors truncate block">
                      {t?.name ?? '—'}
                    </Link>
                    {t?.start_date && (
          ...
          >
            <span className="font-black text-sm">Overall</span>
      ...
      <p className="mt-6 text-xs text-muted-foreground/50 text-center">
        Accuracy counts only resolved picks (correct + wrong). Pending picks excluded from the percentage.
      </p>
    </div>
  )
}
