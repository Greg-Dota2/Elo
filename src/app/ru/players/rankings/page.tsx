...
export const metadata: Metadata = {
  title: 'Рейтинг игроков Dota 2 — турнирные результаты',
  description: 'Рейтинг про-игроков по результатам в турнирах Tier 1 Dota 2 — очки за позицию, призовые и очки EPT.',
  alternates: {
    canonical: '/ru/players/rankings',
    languages: { 'x-default': '/players/rankings', 'en': '/players/rankings', 'ru': '/ru/players/rankings' },
  },
}

const POSITION_LABEL: Record<number, string> = { 1: 'Керри', 2: 'Мид', 3: 'Оффлейн', 4: 'Soft Sup', 5: 'Hard Sup' }
...
export default async function RuPlayerRankingsPage() {
  ...
  return (
    <div className="fade-in-up">
      {/* Header */}
      <div className="mb-6">
        <p className="section-label mb-2">Топ сцена</p>
        <div className="flex items-start justify-between gap-4 flex-wrap mb-3">
          <h1 className="text-3xl font-black tracking-tight">Рейтинг игроков</h1>
          <Link
            href="/ru/players"
        ...
        <p className="text-sm text-muted-foreground leading-relaxed">
          Рейтинг строится по трём факторам в порядке приоритета: <span className="text-primary font-semibold">очки EPT</span> идут
          первыми — они начисляются только на наиболее престижных ивентах цикла и имеют наибольший вес.
          Далее идёт <span className="text-foreground font-semibold">очки за позицию</span> — каждый результат на турнире приносит очки (1-е = 100, 2-е = 75, 3-4-е = 50…),
          умноженные на призовой фонд, поэтому финал на $1M турнире важнее финала на $175K.
          <span className="text-amber-400 font-semibold"> Призовые</span> разрешают оставшиеся ничьи.
          Игроки одной команды всегда идут в порядке: керри → мид → оффлейн → саппорт.
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap mb-8">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary/25 bg-primary/10">
          ...
          <span className="text-sm text-muted-foreground">игроков в рейтинге</span>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-amber-400/25 bg-amber-400/10">
          ...
          <span className="text-sm text-muted-foreground">суммарно распределено</span>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-border/50 bg-secondary/40">
          ...
          <span className="text-sm text-muted-foreground">турниров учтено</span>
        </div>
      </div>

      {ranked.length === 0 ? (
        <div className="rounded-2xl p-12 text-center border border-border/60 bg-card/40">
          <p className="font-semibold mb-1">Рейтинг пока пуст</p>
          <p className="text-sm text-muted-foreground">Добавьте призовые распределения турниров для формирования рейтинга.</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/60 bg-secondary/40">
                  ...
                  <th className="text-left text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3">Игрок</th>
                  <th className="text-left text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3 hidden md:table-cell">Команда</th>
                  <th className="text-left text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3 hidden lg:table-cell">Роль</th>
                  <th className="text-right text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3">Очки</th>
                  <th className="text-right text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3 hidden sm:table-cell">Призовые</th>
                  ...
                  <th className="text-center text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3 hidden sm:table-cell">Лучший рез.</th>
                  <th className="text-center text-[11px] font-bold uppercase tracking-wider text-muted-foreground px-4 py-3 hidden lg:table-cell">Ивенты</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={player.id} className="hover:bg-secondary/30 transition-colors group">
                      ...
                      <td className="px-4 py-3.5">
                        <Link href={`/ru/players/${player.slug}`} className="flex items-center gap-3">
                          {player.photo_url ? (
                      ...
                      <td className="px-4 py-3.5 hidden md:table-cell">
                        {player.team && (
                          <Link
                            href={player.team.slug ? `/ru/teams/${player.team.slug}` : '#'}
                      ...
                      <td className="px-4 py-3.5 text-right">
                        ...
                        <span className="text-xs text-muted-foreground ml-1">очк</span>
                      </td>
